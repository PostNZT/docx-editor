...
A Vue contributor would need to reimplement all of this logic. The goal is to extract the state machines and coordination logic into framework-agnostic classes in `@PostNZT/docx-core`.

## Goals / Non-Goals

**Goals:**

- Extract framework-agnostic state machines and coordination logic from React components into plain TypeScript classes in `@PostNZT/docx-core`
...
### 4. Keep managers in `@PostNZT/docx-core` as plain classes

**Decision:** All manager/coordinator classes live in `packages/core/src/managers/` and are exported from `@PostNZT/docx-core`.
