...
The `packages/core/` directory SHALL contain all framework-agnostic source code with its own `package.json` named `@PostNZT/docx-core`.
...
- **THEN** `@PostNZT/docx-core` SHALL be listed in dependencies
...
- **AND** it SHALL externalize `@PostNZT/docx-core` (not bundle it)
...
- **AND** the app SHALL render a functional DOCX editor using `@PostNZT/docx-editor-vue` and `@PostNZT/docx-core`
...
The `packages/vue/` directory SHALL contain an empty scaffolded package named `@PostNZT/docx-editor-vue` with a dependency on `@PostNZT/docx-core`, ready for community contribution.

#### Scenario: Vue package has minimal package.json

- **WHEN** inspecting `packages/vue/package.json`
- **THEN** it SHALL have name `@PostNZT/docx-editor-vue`
- **AND** `@PostNZT/docx-core` SHALL be listed in dependencies
