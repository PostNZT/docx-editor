...
- `core-package-api`: Public API surface for `@PostNZT/docx-core` — exports for DOCX parsing/serialization, ProseMirror schema/extensions/commands, layout engine, document model types, and headless editor creation
...
- **Downstream**: Vue/Svelte/vanilla consumers can depend on `@PostNZT/docx-core` directly with zero React overhead
