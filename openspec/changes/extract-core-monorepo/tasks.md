...
- [x] 5.3 Export `EditorPluginCore`, `PluginPanelProps`, `PanelConfig`, `RenderedDomContext` from `@PostNZT/docx-core`
...
- [x] 6.1 Update all imports in `packages/react/` that reference core modules to use `@PostNZT/docx-core` instead of relative paths
- [x] 6.2 Update `packages/react/src/index.ts` to re-export everything from `@PostNZT/docx-core` for backwards compatibility
...
- [x] 7.2 Create `packages/react/tsup.config.ts` with entry points (`index.ts`, `react.ts`, `ui.ts`, re-exports) externalizing `@PostNZT/docx-core`
