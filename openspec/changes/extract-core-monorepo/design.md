...
**Decision:** React package imports from `@PostNZT/docx-core` (the npm package name), not relative paths.
...
**Core interface (`@PostNZT/docx-core`):**
...
**Vue adapter (`@PostNZT/docx-editor-vue`):**
...
- `EditorPluginCore`, `PanelConfig`, `PluginPanelProps` (without ReactNode), `RenderedDomContext` → `@PostNZT/docx-core`
...
**[Risk] Internal import churn** → Files that currently do `../docx/parser` will need `@PostNZT/docx-core` imports. Mitigation: batch find-and-replace, verify with typecheck.
