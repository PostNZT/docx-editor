...
```typescript
import pl from '@PostNZT/docx-js-editor/i18n/pl.json';
```
...
- **AND** the file SHALL be usable immediately via `import de from '@PostNZT/docx-js-editor/i18n/de.json'`
...
The package `package.json` SHALL include exports entries for locale JSON files so they can be imported by consumers using the `@PostNZT/docx-js-editor/i18n/{lang}` path.

#### Scenario: Consumer imports a locale

- **WHEN** a consumer writes `import pl from '@PostNZT/docx-js-editor/i18n/pl.json'`
