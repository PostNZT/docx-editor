...
The package SHALL be published as `@PostNZT/docx-editor-agents` on public npm under BSL-1.1. SHALL declare `@PostNZT/docx-core` as a peer dependency. SHALL include a `LICENSE` file with BSL-1.1 terms (licensor: PostNZT, change date: 4 years, change license: MIT, additional use grant: non-production permitted).

#### Scenario: Install and import

- **WHEN** `npm install @PostNZT/docx-editor-agents` is run and the package is imported in Node.js
...
- **WHEN** `import { ReviewChange, ContentBlock } from '@PostNZT/docx-editor-agents'` is used
- **THEN** types are available for type-checking

### Requirement: Optional bridge export for client-side

The package SHALL provide `@PostNZT/docx-editor-agents/bridge` as a separate entry point for React editor integration. The main entry point SHALL NOT import React or DOM code.

#### Scenario: Bridge import

- **WHEN** `import { createReviewBridge } from '@PostNZT/docx-editor-agents/bridge'` is used in a React app
