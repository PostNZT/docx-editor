...
Monorepo: `packages/core` (`@PostNZT/docx-core`, MIT), `packages/react`, `packages/vue`. Root workspace `"packages/*"`. Build: tsup. New package: `packages/agent-use` → `@PostNZT/docx-editor-agents` (BSL-1.1).
...
`packages/agent-use/` with `@PostNZT/docx-core` as peer dep. Published on public npm under BSL-1.1.
