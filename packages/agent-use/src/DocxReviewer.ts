...
import type { Document, DocumentBody } from '@postnzt/docx-core/headless';
import { parseDocx } from '@postnzt/docx-core/headless';
import type {
  ContentBlock,
  GetContentOptions,
  ReviewChange,
...
export class DocxReviewer {
   ...
   * @param document - Parsed Document from @postnzt/docx-core
  ...
  async toBuffer(): Promise<ArrayBuffer> {
    ...
    const { repackDocx } = await import('@postnzt/docx-core/headless');
