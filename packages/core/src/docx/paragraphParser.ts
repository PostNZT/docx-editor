    ...
    if (numId !== undefined && numId !== 0) {
      ...
      if (level) {
        ...
        if (level.pPr) {
          ...
          // and `hanging="0"` as no-op so the numbering level's indent still
          // applies. A non-numeric value parses to NaN and falls through as
          // an override, preserving prior behavior on malformed input.
