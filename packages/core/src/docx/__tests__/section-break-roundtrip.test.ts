...
// break is stored on a paragraph via `w:pPr/w:sectPr`. The parser captured it
// onto `Paragraph.sectionProperties`, but the serializer never wrote it back,
// so a headless parseDocx → repackDocx roundtrip collapsed two sections into
// the final `w:body/w:sectPr` only.
